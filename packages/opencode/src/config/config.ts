    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const envConfigContent = process.env.SPRINT_CONFIG_CONTENT ?? process.env.MERGE_CONFIG_CONTENT
          if (envConfigContent) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(envConfigContent, {
                dir: ctx.directory,
                source: process.env.SPRINT_CONFIG_CONTENT ? "SPRINT_CONFIG_CONTENT" : "MERGE_CONFIG_CONTENT",
              }),
            )
            log.debug("loaded custom config from environment")
          }
